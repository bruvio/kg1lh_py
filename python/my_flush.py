...
try:
    lib_flush = CDLL(os.path.dirname(cwd)+"/libflush.so")  # 64b compiled library
...
# ##############################
    ...
    def flushquickinit(ishot, time):
        ...
        lib_flush.flushquickinit_(byref(ishot), byref(time), byref(ier))
    ...
    def fluqax(nQ, q):
        ...
        ierr = c_int(0)
        lib_flush.fluqax_(byref(nQ), q, byref(nOut), psi, rInner, rOuter, byref(ierr))
    ...
    def Flush_getClosedFluxSurface(flux, nPoints=360):
        ...
        ier = c_int(0)
        lib_flush.Flush_getClosedFluxSurfaces(
            byref(nflux), flux, byref(nPoints), r, z, byref(ier)
        )
    ...
    def Flush_getLCFSboundary(accuracy=1e-4):
        ...
        lib_flush.Flush_getLCFSboundary(
            byref(nLCFS), byref(rLCFS), byref(zLCFS), byref(accuracy), byref(ier)
        )
    ...
    def Flush_readFirstWall():
        ...
        nn = c_int(0)
        ier = c_int(0)
        lib_flush.flush_readfirstwall_(byref(nn), rLim, zLim, byref(ier))
        if ier.value == 0:
            rLim = rLim[: nn.value]
            zLim = zLim[: nn.value]
        else:
    ...
    def Flush_writeEQDSKfile(filename, nR, nZ):
        ...
        lib_flush.flush_writeeqdskfile_(
            filename, byref(nR), byref(nZ), byref(ier), filename_len
        )
        return ier.value
    ...
    def Flush_getSplineDomainLimits():
        ...
        zMax = c_double(0.0)
        ier = c_int(0)
        lib_flush.flush_getsplinedomainlimits_(
            byref(rMin), byref(zMin), byref(rMax), byref(zMax), byref(ier)
        )
    ...
    def Flush_getTangentsToSurfaces(r, z, flux, iside, accuracy, nBeams):
        ...
        lib_flush.flush_gettangentstosurfaces_(
            r, z, angle, nflux, flux, iside, rTan, zTan, accuracy, nBeams, ier
        )
    ...
    lib_flush.flush_getxpoint_.argtypes = [
        POINTER(c_int),  # nX
        ndpointer(dtype=np.float64),  # rX,
        ndpointer(dtype=np.float64),  # zX,
        ndpointer(dtype=np.float64),  # fX,
        POINTER(c_int),
    ]  # ier


    def flush_getXpoint():
        rx = np.empty(3, dtype=np.float64)
        zx = np.empty(3, dtype=np.float64)
        fx = np.empty(3, dtype=np.float64)
        nx = c_int(0)
        ier = c_int(0)

        lib_flush.flush_getxpoint_(byref(nx), rx, zx, fx, byref(ier))
    ...
    def flush_blowUpSurface(psi, delta, nSurf, rSurf, zSurf):
        ...
        nSurf = c_int(nSurf)
        rSurf = np.ascontiguousarray(np.atleast_1d(rSurf), dtype=np.float64)
        zSurf = np.ascontiguousarray(np.atleast_1d(zSurf), dtype=np.float64)
        ierr = c_int(0)

        lib_flush.flush_blowupsurface_(psi, delta, byref(nSurf), rSurf, zSurf, byref(ierr))
        return ierr.value
    ...
    lib_flush.flush_gettangentflux_.argtypes = [
        ...
        ndpointer(dtype=np.float64),  # rTan
        ndpointer(dtype=np.float64),  # zTan
        ndpointer(dtype=np.float64),  # fTan
        POINTER(c_int),
    ]  # ier
    ...
    def Flush_GetTangentFlux(r, z, angle, accuracy):
        ...
        accuracy = c_double(accuracy)
        fTan = np.empty(1, dtype=np.float64)
        rTan = np.empty(1, dtype=np.float64)
        zTan = np.empty(1, dtype=np.float64)
        ier = c_int(0)
