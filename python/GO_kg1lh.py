...
def map_kg1_efit_RM_pandas(arg):
    ...
    dummy = data.KG1LH_data2.lid[chan].resample_signal("interp", tefit)
    ...
    data.KG1LH_data2.lid[chan].time = tefit
...
def map_kg1_efit_RM(arg):
    ...
    # data.KG1LH_data.lid[chan].time = tefit
    dummy = data.KG1LH_data1.lid[chan].resample_signal("interp", tefit)
    data.KG1LH_data1.lid[chan].time = np.empty(ntefit)

    data.KG1LH_data1.lid[chan].data = dummy
    data.KG1LH_data1.lid[chan].time = tefit
...
def time_loop(arg):
    ...
    for IT in range(0, ntefit):
        ...
        # rTan1,zTan1, fTan1, ier  = Flush_GetTangentFlux(xpt,ypt,delta, data.EPSDD)
        if ier != 0:
            logger.error('flush error {} in Flush_GetTangentFlux'.format(ier))
            return ier
        #
        # if int(iflsep) != 0:
        #     print('\n')
...
# C-----------------------------------------------------------------------
    ...
    # #
    # logger.info("\n             dumping data to pickle.\n")
    # with open('./test_data_kg1l_cm.pkl', 'wb') as f:
    #     pickle.dump(
    #         [data.EFIT_data, data.KG1_data,
    #          data.KG1LH_data,data.KG1LH_data1,data.KG1LH_data2], f)
    # f.close()
