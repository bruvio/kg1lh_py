...
def map_kg1_efit_RM_pandas(arg):
    ...
    data.KG1LH_data.lid[chan].time = [
        np.float64(i) for i in data.KG1_data.density[chan].time
    ]
    ...
    data.KG1LH_data.lid[chan].data = np.array(dummy, dtype=np.float64)
    data.KG1LH_data.lid[chan].time = np.array(dummy_time, dtype=np.float64)
...
def map_kg1_efit_RM(arg):
    ...
    data.KG1LH_data.lid[chan].time = data.KG1_data.density[
        chan
    ].time  # [np.float64(i) for i in data.KG1_data.density[chan].time]
    ...
    if data.code.lower() == "kg1h":
        ...
        data.KG1LH_data.lid[chan].data = np.array(dummy)
        data.KG1LH_data.lid[chan].time = np.array(dummy_time)
...
def map_kg1_efit(arg):
    ...
    data.KG1LH_data.lid[chan] = SignalBase(data.constants)
    data.KG1LH_data.lid[chan].data = np.array(density)
    data.KG1LH_data.lid[chan].time = np.array(time_efit)
    # data.KG1LH_data.lid[chan].time = time_efit
    if data.interp_method == "interp":
        dummy, dummy_time = data.KG1LH_data.lid[chan].resample_signal(
            data.interp_method, time_efit
...
def time_loop(arg):
    ...
    for IT in range(0, ntefit):
        ...
        if length[IT] > 0.0:
            lad[IT] = float(data.KG1LH_data.lid[chan].data[IT] / length[IT])
        else:
            lad[IT] = 0.0
    ...
    data.KG1LH_data.lid[chan] = SignalBase(data.constants)
    data.KG1LH_data.lid[chan].data = np.array(
        data.KG1LH_data.lid[chan].data, dtype=np.float64
    )
    ...
    if data.code.lower() == "kg1h":
        data.KG1LH_data.lad[chan].data = np.array(
            lad, dtype=np.float64
        )  # [float(i) for i in lad]
        data.KG1LH_data.len[chan].data = np.array(
            length, dtype=np.float64
        )  # [float(i) for i in length]
        data.KG1LH_data.xta[chan].data = np.array(
            xtan, dtype=np.float64
        )  # [float(i) for i in xtan]
        ...
        data.KG1LH_data.len[chan].data = np.array(
            length, dtype=np.float
        )  # [float(i) for i in length]
        data.KG1LH_data.xta[chan].data = np.array(
            xtan, dtype=np.float
        )  # [float(i) for i in xtan]
        data.KG1LH_data.lid[chan].time = [np.float(i) for i in time_efit]
        data.KG1LH_data.lad[chan].time = [np.float(i) for i in time_efit]
        data.KG1LH_data.len[chan].time = [np.float(i) for i in time_efit]
...
def main(
    ...
    try:
        ...
        (
            ...
            data.data_coord,
            data.coord_err,
        ) = data.KG1_data.get_coord(data.pulse)
        ...
        try:
            ...
            for i, r in enumerate(results):
                if len(r[0].KG1LH_data.lid.keys()) != 0:
                    ...
                    data.KG1LH_data.lid[i + 1].time = [
                        float(i) for i in r[0].KG1LH_data.lid[r[1]].time
                    ]  # r[0].KG1LH_data.lid[r[1]].time
                    data.KG1LH_data.lid[i + 1].data = [
                        float(i) for i in r[0].KG1LH_data.lid[r[1]].data
                    ]  # r[0].KG1LH_data.lid[r[1]].data
                else:
                    continue
        except:
    ...
    #
        ...
        try:
            ...
            with Pool(10) as pool:
                results = pool.map(
                    map_kg1_efit_RM_pandas, [(data, chan) for chan in channels]
                )
            logger.info("--- {}s seconds ---".format((time.time() - start_time)))
    ...
    try:
        ...
        for i, res in enumerate(results):
            if len(res[0].KG1LH_data.lad.keys()) != 0:
                ...
                data.KG1LH_data.lad[i + 1].time = [
                    float(i) for i in res[0].KG1LH_data.lad[res[1]].time
                ]
                data.KG1LH_data.lad[i + 1].data = [
                    float(i) for i in res[0].KG1LH_data.lad[res[1]].data
                ]  # res[0].KG1LH_data.lad[res[1]].data

                data.KG1LH_data.len[i + 1] = SignalBase(data.constants)
                data.KG1LH_data.len[i + 1].time = [
                    float(i) for i in res[0].KG1LH_data.len[res[1]].time
                ]  # res[0].KG1LH_data.len[res[1]].time
                data.KG1LH_data.len[i + 1].data = [
                    float(i) for i in res[0].KG1LH_data.len[res[1]].data
                ]  # res[0].KG1LH_data.len[res[1]].data

                data.KG1LH_data.xta[i + 1] = SignalBase(data.constants)
                data.KG1LH_data.xta[i + 1].time = [
                    float(i) for i in res[0].KG1LH_data.xta[res[1]].time
                ]  # res[0].KG1LH_data.xta[res[1]].time
                data.KG1LH_data.xta[i + 1].data = [
                    float(i) for i in res[0].KG1LH_data.xta[res[1]].data
                ]  # res[0].KG1LH_data.xta[res[1]].data
            else:
                continue
    except:
    ...
    if (write_uid != "" and not test) or (
        ...
        if write_err != 0:
            logger.error(
                "Failed to write {}/{}. Errorcode {}".format(dda, dtype_mode, write_err)
            )
            return 67
